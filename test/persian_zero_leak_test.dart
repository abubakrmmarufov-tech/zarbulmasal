...
void main() {
  ...
  group('Persian Zero-Leak & Translation Parity', () {
    test('AppTranslations.fa has 100% key symmetry with AppTranslations.tj', () {
    ...
    test('AppTranslations.fa values contain strictly 0 Cyrillic characters', () {
      final leaks = <String, String>{};

      for (final entry in AppTranslations.fa.entries) {
        expect(entry.value.trim(), isNotEmpty, reason: 'Empty translation for key ${entry.key}');
    ...
    test('assets/data/history/books.json Persian fields contain strictly 0 Cyrillic characters', () {
      ...
      expect(books.length, equals(7), reason: 'Expected 7 history textbooks (grades 5-11)');
      ...
      for (final book in books) {
        ...
        expect(titlePersian, isNotNull, reason: 'Missing titlePersian in book $id');
        expect(titlePersian!.trim(), isNotEmpty, reason: 'Empty titlePersian in book $id');
        expect(cyrillicRegex.hasMatch(titlePersian), isFalse,
            reason: 'Cyrillic leak in book $id titlePersian: $titlePersian');

        expect(authorPersian, isNotNull, reason: 'Missing authorPersian in book $id');
        expect(authorPersian!.trim(), isNotEmpty, reason: 'Empty authorPersian in book $id');
        expect(cyrillicRegex.hasMatch(authorPersian), isFalse,
            reason: 'Cyrillic leak in book $id authorPersian: $authorPersian');

        expect(descPersian, isNotNull, reason: 'Missing descriptionPersian in book $id');
        expect(descPersian!.trim(), isNotEmpty, reason: 'Empty descriptionPersian in book $id');
        expect(cyrillicRegex.hasMatch(descPersian), isFalse,
            reason: 'Cyrillic leak in book $id descriptionPersian: $descPersian');
      }
    });

    test('assets/data/history/entries.json Persian fields contain strictly 0 Cyrillic characters', () {
      ...
      expect(entries.length, equals(85), reason: 'Expected 85 verified history entries');
      ...
      for (final entry in entries) {
        ...
        expect(titlePersian, isNotNull, reason: 'Missing titlePersian in entry $id');
        expect(titlePersian!.trim(), isNotEmpty, reason: 'Empty titlePersian in entry $id');
        if (cyrillicRegex.hasMatch(titlePersian)) {
          leaks.add('$id: titlePersian -> $titlePersian');
        }

        expect(summaryPersian, isNotNull, reason: 'Missing summaryPersian in entry $id');
        expect(summaryPersian!.trim(), isNotEmpty, reason: 'Empty summaryPersian in entry $id');
        ...
        if (significancePersian != null && cyrillicRegex.hasMatch(significancePersian)) {
        ...
        if (capitalPersian != null && cyrillicRegex.hasMatch(capitalPersian)) {
          leaks.add('$id: capitalPersian -> $capitalPersian');
        }

        if (territoryPersian != null && cyrillicRegex.hasMatch(territoryPersian)) {
          leaks.add('$id: territoryPersian -> $territoryPersian');
        }

        if (founderPersian != null && cyrillicRegex.hasMatch(founderPersian)) {
          leaks.add('$id: founderPersian -> $founderPersian');
        }

        if (religionPersian != null && cyrillicRegex.hasMatch(religionPersian)) {
          leaks.add('$id: religionPersian -> $religionPersian');
        }

        if (originsPersian != null && cyrillicRegex.hasMatch(originsPersian)) {
          leaks.add('$id: originsPersian -> $originsPersian');
        }

        if (culturePersian != null && cyrillicRegex.hasMatch(culturePersian)) {
          leaks.add('$id: culturePersian -> $culturePersian');
        }

        if (declinePersian != null && cyrillicRegex.hasMatch(declinePersian)) {
          leaks.add('$id: declinePersian -> $declinePersian');
        }

        final keyFiguresPersian = (entry['keyFiguresPersian'] as List<dynamic>?)?.cast<String>() ?? [];
        ...
        final rulersPersian = (entry['rulersPersian'] as List<dynamic>?)?.cast<String>() ?? [];
        ...
        final claimProvenance = (entry['claimProvenance'] as List<dynamic>?)?.cast<Map<String, dynamic>>() ?? [];
      ...
      expect(leaks, isEmpty, reason: 'Found Cyrillic leaks in history entries: $leaks');
    });

    test('assets/data/literature/poets.json Persian fields contain strictly 0 Cyrillic characters', () {
      ...
      expect(poets.length, equals(150), reason: 'Expected exactly 150 poets in catalog');
      ...
      for (final poet in poets) {
        ...
        expect(namePersian, isNotNull, reason: 'Missing canonicalNamePersian in poet $id');
        expect(namePersian!.trim(), isNotEmpty, reason: 'Empty canonicalNamePersian in poet $id');
        ...
        expect(bioFa!.trim(), isNotEmpty, reason: 'Empty biographyFa in poet $id');
    ...
    test('assets/data/literature/works.json Persian fields contain strictly 0 Cyrillic characters and scriptSource is both', () {
      ...
      expect(works.length, equals(1472), reason: 'Expected exactly 1472 literary works in catalog');
      ...
      for (final work in works) {
        ...
        expect(titlePersian, isNotNull, reason: 'Missing titlePersian in work $id');
        expect(titlePersian!.trim(), isNotEmpty, reason: 'Empty titlePersian in work $id');
        if (cyrillicRegex.hasMatch(titlePersian)) {
          leaks.add('$id: titlePersian -> $titlePersian');
        }

        expect(textPersian, isNotNull, reason: 'Missing textPersian in work $id');
        expect(textPersian!.trim(), isNotEmpty, reason: 'Empty textPersian in work $id');
        if (cyrillicRegex.hasMatch(textPersian)) {
          leaks.add('$id: textPersian -> ${textPersian.substring(0, textPersian.length > 50 ? 50 : textPersian.length)}');
        }

        if (scriptSource == 'both') {
          countBoth++;
        }
      }

      expect(leaks, isEmpty, reason: 'Found Cyrillic leaks in works: $leaks');
      expect(countBoth, equals(1472), reason: 'All 1472 works must have scriptSource: "both"');
    });

    test('seedProverbs contains 150 verified proverbs with 0 Cyrillic characters in persianText', () {
      expect(seedProverbs.length, equals(150), reason: 'Expected 150 verified production proverbs');

      final leaks = <String>[];

      for (final proverb in seedProverbs) {
        expect(proverb.persianText.trim(), isNotEmpty,
      ...
      expect(leaks, isEmpty, reason: 'Found Cyrillic leaks in seedProverbs: $leaks');
    });
  });
}
