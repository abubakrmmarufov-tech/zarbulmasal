...
void main() {
  ...
  group('Literature Providers', () {
    ...
    test('literatureRepositoryProvider provides LiteratureRepository instance', () {
    ...
    test('approvedWorksProvider filters works to only displayable ones', () async {
    ...
    test('dailyVerseProvider returns null when no approved works exist', () async {
    ...
    test('authorByIdProvider finds author by id', () async {
      ...
      final unknown = await container.read(authorByIdProvider('unknown-id').future);
      expect(unknown, isNull);
    });

    test('schoolCanonByAuthorProvider finds canon by author id', () async {
      final rudakiCanon =
          await container.read(schoolCanonByAuthorProvider('rudaki').future);
    ...
    test('worksByAuthorProvider filters works by author id', () async {
      final works =
          await container.read(worksByAuthorProvider('rudaki').future);
      expect(works, isEmpty);
    });
  });

  group('dailyVerseProvider with approved works override', () {
    test('returns deterministic daily verse when approved works are present', () async {
      final testWork = LiteraryWork(
        id: 'test-daily-work',
        authorId: 'rudaki',
  ...
  group('LiteraryFavoritesNotifier', () {
    ...
    test('toggle adds and removes work IDs in state and SharedPreferences', () async {
      SharedPreferences.setMockInitialValues({});
      final notifier = LiteraryFavoritesNotifier();
      await notifier.loadFavorites();
      ...
      expect(prefs.getStringList(AppConstants.prefsLiteraryFavorites), ['work-1']);

      await notifier.toggle('work-1');
      expect(notifier.state, isEmpty);
      expect(notifier.isFavorite('work-1'), isFalse);
      expect(prefs.getStringList(AppConstants.prefsLiteraryFavorites), isEmpty);
    });
    ...
    test('literaryFavoritesProvider and literaryFavoriteWorksProvider integrate correctly', () async {
      SharedPreferences.setMockInitialValues({
        AppConstants.prefsLiteraryFavorites: ['fav-work-1'],
      });

      final favWork = LiteraryWork(
        id: 'fav-work-1',
        authorId: 'rudaki',
      ...
      final container = ProviderContainer(
        overrides: [
          literaryWorksProvider.overrideWith((ref) async => [favWork, nonFavWork]),
        ],
      );
      addTearDown(container.dispose);

      final favoritesNotifier = container.read(literaryFavoritesProvider.notifier);
      await favoritesNotifier.loadFavorites();

      final favorites = container.read(literaryFavoritesProvider);
      expect(favorites, {'fav-work-1'});

      final favWorks = await container.read(literaryFavoriteWorksProvider.future);
