...
# Runs a generator and fails if it changed [file] from what was on disk,
# committed or not; the regenerated file is left in place.
generated_in_sync() {
  local file="$1" before
  shift
  before="$(mktemp -t check-before.XXXXXX)"
  cp "$file" "$before"
  "$@" || return 1
  diff -q "$before" "$file" >/dev/null && return 0
  printf '%s was out of date; regenerated by: %s\n' "$file" "$*"
  return 1
}

runtime_catalog_in_sync() {
  generated_in_sync assets/data/literature/runtime_works.json \
    dart run tool/build_runtime_literature.dart
}

source_inventory_in_sync() {
  generated_in_sync docs/literature/SOURCE_INVENTORY.md \
